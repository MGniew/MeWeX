...
do
	...
	output_path="${input_dir_base}/${corpus}_${ratio}/${crp}"
	mkdir -p ${output_path}
	${tool} -i ${crp_path} -o ${output_path} -e 1 -r ${relevant_set} -p ${value}
	
	output_path="${input_dir_base}/${corpus}_${ratio}/${crp}_morfeusz"
	mkdir -p ${output_path}
	${tool} -i ${crp_path} -o ${output_path} -e 1 -r ${relevant_set} -p ${value} -f ${morfeusz}
	
	output_path="${input_dir_base}/${corpus}_${ratio}/${crp}_morfeuszFreq5"
	mkdir -p ${output_path}
