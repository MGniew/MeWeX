...
output_dir_base='/home/michalw/dane/results/kipi_3_new_scheme_nf'
...
#non_param_bigram_functions="-a frequency() -a ef() -a ief() -a jaccard() -a dice() -a sorgenfrei() -a or() -a us() -a ct1() -a ct2() -a me() -a sc() -a wsc() -a smd() -a sfbmd() -a tscore() -a zscore() -a px2() -a wx2() -a g2() -a fdpn(sec(e=4.2)) -a abg(sec(e=4.2)) -a sbg(sec(e=4.2)) -a mbg(sec(e=4.2))"
non_param_bigram_functions="-a fdpn(inner=sec(e=4.2)) -a abg(inner=sec(e=4.2)) -a sbg(inner=sec(e=4.2)) -a mbg(inner=sec(e=4.2))"
...
then
	...
	#echo 'Okno # Okno' 
	#crp='3w'
	#stat="relation(^,file=${relation_dir}/3w.csv)"
	#extr="relation(^,file=${relation_dir}/3w.csv)"
	#out_dir="${output_dir_base}/3_okno_okno_${crp}"
	#mkdir -p ${out_dir}
	...
	#crp='3w'
	#stat="and(relation(^,file=${relation_dir}/3w.csv),every_word(^,file=${morfeusz}))"
	#extr="and(relation(^,file=${relation_dir}/3w.csv),every_word(^,file=${morfeusz}))"
	#out_dir="${output_dir_base}/3_okno_okno_${crp}_morfeusz"
	#mkdir -p ${out_dir}
	#order="-a w_order(scheme=${corpus}/${crp}/scheme.csv) -a w_tf_order(scheme=${corpus}/${crp}/scheme.csv)"
	#${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}
