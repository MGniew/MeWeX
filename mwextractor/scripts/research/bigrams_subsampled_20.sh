...
corpus="${input_dir_base}/kipi_sub20"
output_dir_base='/home/michalw/dane/results/kipi_sub20'
...
then
	...
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2w/scheme.csv) -a w_tf_order(scheme=${corpus}/2w/scheme.csv)"
	#${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}
	${command} -I ${corpus}/${crp} -O ${out_dir} ${order}
	...
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2w1h/scheme.csv) -a w_tf_order(scheme=${corpus}/2w1h/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} ${order}
	...
	crp="2w1h_morfeuszFreq${frequency}"
...
then
	...
	crp='2r_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2r.csv))"
	extr=${stat}
	out_dir="${output_dir_base}/2_relacje_relacje_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2r/scheme.csv) -a w_tf_order(scheme=${corpus}/2r/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}

	echo 'Relacje, okno, morfeusz # Relacje'
	crp='2rw_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2rw.csv))"
	extr="and(relation(^,file=${relation_dir}/2r.csv))"
	out_dir="${output_dir_base}/2_relacjeOkno_relacje_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2rw/scheme.csv) -a w_tf_order(scheme=${corpus}/2rw/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}

	echo 'Relacje, dziura, morfeusz # Relacje, dziura'
	crp='2r1h_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2r1h.csv))"
	extr="and(relation(^,file=${relation_dir}/2r1h.csv))"
	out_dir="${output_dir_base}/2_relacjeDziura_relacje_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2r1h/scheme.csv) -a w_tf_order(scheme=${corpus}/2r1h/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}

	echo 'Relacje, dziura, morfeusz, okno # Relacje, dziura'
	crp='2rw1h_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2rw1h.csv))"
	extr="and(relation(^,file=${relation_dir}/2r1h.csv))"
	out_dir="${output_dir_base}/2_relacjeOknoDziura_relacje_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2rw1h/scheme.csv) -a w_tf_order(scheme=${corpus}/2rw1h/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}
	...
	crp='2r_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2r.csv))"
	extr="and(relation(^,file=${relation_dir}/f2r.csv))"
	out_dir="${output_dir_base}/2_relacje_relacjeCzeste_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2r/scheme.csv) -a w_tf_order(scheme=${corpus}/2r/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}
	...
	crp='2r1h_morfeusz'
	stat="and(relation(^,file=${relation_dir}/2r1h.csv))"
	extr="and(relation(^,file=${relation_dir}/f2r1h.csv))"
	out_dir="${output_dir_base}/2_relacjeDziura_relacjeCzeste_${crp}"
	mkdir -p ${out_dir}
	order="-a w_order(scheme=${corpus}/2r1h/scheme.csv) -a w_tf_order(scheme=${corpus}/2r1h/scheme.csv)"
	${command} -I ${corpus}/${crp} -O ${out_dir} -s ${stat} -x ${extr} ${order}
...
then
	...
	crp="2rw_morfeuszFreq${frequency}"
	stat="and(relation(^,file=${relation_dir}/2rw.csv))"
	extr="and(relation(^,file=${relation_dir}/2r.csv))"
	out_dir="${output_dir_base}/2_relacjeOkno_relacje_${crp}"
	mkdir -p ${out_dir}
	...
	crp="2r1h_morfeuszFreq${frequency}"
	stat="and(relation(^,file=${relation_dir}/2r1h.csv))"
	extr="and(relation(^,file=${relation_dir}/2r1h.csv))"
	out_dir="${output_dir_base}/2_relacjeDziura_relacje_${crp}"
	mkdir -p ${out_dir}
	...
	crp="2rw1h_morfeuszFreq${frequency}"
	stat="and(relation(^,file=${relation_dir}/2rw1h.csv))"
	extr="and(relation(^,file=${relation_dir}/2r1h.csv))"
	out_dir="${output_dir_base}/2_relacjeOknoDziura_relacje_${crp}"
	mkdir -p ${out_dir}
